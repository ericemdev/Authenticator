<script>
    ...
    const toggleDarkMode = () => {
        ...
        showPopup = false;
        notification = darkMode ? 'Switched to Dark Mode' : 'Switched to Light Mode';
        setTimeout(() => (notification = ''), 3000);
    };

    async function syncWithGoogle() {
        try {
            // Add logic for syncing with Google
            notification = 'Google Sync Successful';
        } catch (error) {
            notification = 'Google Sync Failed';
        }
        setTimeout(() => (notification = ''), 3000);
    }
...
<aside class="{darkMode ? 'dark:bg-[#15202b] text-secondary' : 'bg-primary text-gray-800'} hidden lg:flex flex-col justify-between w-1/5 min-h-screen shadow-lg p-4">
    <div>
        ...
        <ul class="space-y-4 mt-4">
            <li class="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-yellow-100 border-b cursor-pointer dark:border-[#121212]">
                <button on:click={syncWithGoogle} class="w-full flex items-center space-x-2" aria-label="Sync with Google">
                    <GoogleSyncButton />
            ...
            <li class="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 border-b dark:border-[#121212]">
                <button on:click={restore} class="w-full text-left text-lg" aria-label="Import Data">Import</button>
            </li>
            <li class="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 border-b dark:border-[#121212]">
                <button on:click={backup} class="w-full text-left text-lg" aria-label="Export Data">Export</button>
            </li>
        </ul>
    </div>
    <div class="flex justify-start items-center pt-4 pl-6 border-t border-gray-300 dark:border-[#121212]">
        <label class="relative inline-flex items-center cursor-pointer">
            ...
            <span class="ml-3  font-medium text-lg text-gray-900 dark:text-gray-300">
...
<!-- Notification Popup -->
{#if notification}
    <div class="fixed top-4 right-4 bg-blue-600 text-white px-4 py-2 rounded shadow-lg">
        {notification}
    </div>
{/if}
...
<header class="{darkMode ? 'bg-primaryDark text-white' : 'bg-primary text-[#0d0c22]'} lg:hidden w-full flex items-center justify-between p-4 shadow">
    ...
    <button on:click={togglePopup} class="text-2xl {darkMode ? 'text-white' : 'text-[#0d0c22]'}" aria-label="Toggle Navigation">&#9776;</button>
</header>
...
{#if showPopup}
    <div class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center lg:hidden">
        <div class="{darkMode ? 'bg-gray-900 text-white' : 'bg-primary text-gray-800'} w-3/4 p-4 rounded-lg shadow-lg">
            <button on:click={togglePopup} class="text-gray-500 dark:text-gray-400 self-end text-2xl" aria-label="Close Menu">&times;</button>
            <ul class="space-y-4 mt-4">
                <li class="flex items-center space-x-2 p-2 rounded-md border-b dark:border-black">
                    <button on:click={syncWithGoogle} class="w-full flex items-center space-x-2" aria-label="Sync with Google">
                        <GoogleSyncButton />
                    </button>
                </li>
                <li class="flex items-center space-x-2 p-2 rounded-md border-b dark:border-[#121212]">
                    <button on:click={restore} class="w-full text-left" aria-label="Import Data">Import</button>
                </li>
                <li class="flex items-center space-x-2 p-2 rounded-md border-b dark:border-[#121212]">
                    <button on:click={backup} class="w-full text-left" aria-label="Export Data">Export</button>
                </li>
